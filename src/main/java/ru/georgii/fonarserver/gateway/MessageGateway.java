...
public class MessageGateway {
    ...
    private Map<Long, Set<SocketIOClient>> connections;


    @Autowired
    public MessageGateway(SocketIOServer server) {
        connections = new HashMap<>();
        this.namespace = server.addNamespace("/");
        this.namespace.addConnectListener(onConnected());
        this.namespace.addDisconnectListener(onDisconnected());
        this.namespace.addEventListener("startedTyping", Long.class, onStartedTyping());
        this.namespace.addEventListener("stoppedTyping", Long.class, onStoppedTyping());
        this.namespace.addEventListener("seenMessage", MessageInfo.class, onSeenMessage());
        server.start();
    }

    private DataListener<Long> onStartedTyping() {
        return (client, data, ackSender) -> {
            String saltedGuid = client.getHandshakeData().getHttpHeaders().get("authorization");
            try {
                User u = authService.authenticateBySaltedGuid(saltedGuid);
                if (connections.containsKey(u.getId())) {
                    long uidTo = data;
                    if (connections.containsKey(uidTo)) {
                        for (SocketIOClient conn : connections.get(uidTo)) {
                            conn.sendEvent("startedTyping", u.getId());
                        }
                    }
                }
            } catch (Exception e) {
    ...
    private DataListener<Long> onStoppedTyping() {
        return (client, data, ackSender) -> {
            String saltedGuid = client.getHandshakeData().getHttpHeaders().get("authorization");
            try {
                User u = authService.authenticateBySaltedGuid(saltedGuid);
                if (connections.containsKey(u.getId())) {
                    ...
                    if (connections.containsKey(uidTo)) {
                        for (SocketIOClient conn : connections.get(uidTo)) {
                            conn.sendEvent("stoppedTyping", u.getId());
                        }
                    }
                }
            } catch (Exception e) {
    ...
    private DataListener<MessageInfo> onSeenMessage() {
        return (client, data, ackSender) -> {
            String saltedGuid = client.getHandshakeData().getHttpHeaders().get("authorization");
            try {
                User u = authService.authenticateBySaltedGuid(saltedGuid);
                long messageId = data.messageId;
    ...
    private ConnectListener onConnected() {
        return client -> {
            ...
            String saltedGuid = handshakeData.getHttpHeaders().get("authorization");
            ...
            try {
                ...
                System.out.println("Client[{" + client.getSessionId() + "}] - Authorized user id" + u.getId() + " '{}'");
            } catch (Exception e) {
                client.disconnect();
                System.out.println("Client[{" + client.getSessionId() + "}] - Authorization denied '{}'");
            }
    ...
    public void notifyAboutNewMessage(Message m) {
        if (connections.containsKey(m.getToUserId())) {
            try {
                for (SocketIOClient c : connections.get(m.getToUserId())) {
                    c.sendEvent("message", new ObjectMapper().writeValueAsString(m));
                }
            } catch (Exception e) {
    ...
    public void notifyAboutSeen(Message m) {
        if (connections.containsKey(m.getToUserId())) {
            try {
                for (SocketIOClient c : connections.get(m.getToUserId())) {
                    c.sendEvent("messageSeen", m.getId(), m.getFromUserId());
                }
            } catch (Exception e) {
                System.out.println("Failed to send notification");
            }
...
//public class MessageGateway extends HttpServlet {
//
...
//
//
//        server = new SocketIoServer(mEngineIoServer);
//        ns = server.namespace("/");
//
//        connections = new HashMap<>();
//
